...
Hierarchy: **Workspace → Project → Environment → Service → Deployment**. Most CLI commands use the linked project/environment/service context. Use `railway status --json` to see it; use `--project`, `--environment`, `--service` flags to override.

## GDAI-specific networking conventions

All 17 GDAI services communicate over the **private Railway network**. Only 3 are public-facing: `agentic-web`, `chatwoot-web`, `langfuse-web`.

```
# Inter-service URLs — always use the private domain reference variable:
http://${{ agent-gateway.RAILWAY_PRIVATE_DOMAIN }}:8000
http://${{ otel-collector.RAILWAY_PRIVATE_DOMAIN }}:4318
http://${{ langfuse-web.RAILWAY_PRIVATE_DOMAIN }}:3000
```

**NEVER** use a public Railway domain (`.up.railway.app`) for service-to-service calls — it routes through the public internet and adds ~100ms latency + unnecessary egress cost.

**NEVER** hard-code IPs — Railway containers get new IPs on redeploy. Always use `${{ Service.RAILWAY_PRIVATE_DOMAIN }}`.

**NEVER** put `RAILWAY_TOKEN` or Railway API keys in env vars that are also set as `NEXT_PUBLIC_*` — they're browser-visible.
...
## GDAI service inventory

Key services and their private addresses (reference vars):

| Service | Private domain var | Purpose |
|---|---|---|
| `agent-gateway` | `${{ agent-gateway.RAILWAY_PRIVATE_DOMAIN }}:8000` | FastAPI |
| `langflow-runtime` | `${{ langflow-runtime.RAILWAY_PRIVATE_DOMAIN }}:7860` | Langflow 1.9 |
| `otel-collector` | `${{ otel-collector.RAILWAY_PRIVATE_DOMAIN }}:4318` | OTLP ingest |
| `langfuse-web` | `${{ langfuse-web.RAILWAY_PRIVATE_DOMAIN }}:3000` | Langfuse OSS |
| `n8n` | `${{ n8n.RAILWAY_PRIVATE_DOMAIN }}:5678` | MCP tools |
| `chatwoot-web` | `${{ chatwoot-web.RAILWAY_PRIVATE_DOMAIN }}:3000` | HITL bridge |
| `pg-langflow` | `${{ pg-langflow.RAILWAY_PRIVATE_DOMAIN }}:5432` | Langflow state DB |
| `pg-main` | `${{ pg-main.RAILWAY_PRIVATE_DOMAIN }}:5432` | Supabase-linked |
